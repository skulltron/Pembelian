...
<col style="width: 111px">
  ...
  <?php
    $no=1;
    if($q==""){
        $sql="SELECT * FROM tbjurnal INNER JOIN tbsupplier ON tbjurnal.id_supplier=tbsupplier.id_supplier;";
    }else{
        $sql="SELECT * FROM tbjurnal INNER JOIN tbsupplier ON tbjurnal.id_supplier=tbsupplier.id_supplier WHERE no_bukti LIKE '%q%'";
    }
  ...
  <tr>
    ...
    <td class="tg-0pky">Rp. <?php echo number_format(($row['potongan']),0,'.','.')?></td>
    <td class="tg-0pky">Rp. <?php echo number_format(($row['penjualan']),0,'.','.')?></td>
    <td class="tg-0pky">Rp. <?php echo number_format(($row['piutang_dg']),0,'.','.')?></td>
    <td class="tg-0pky"><?php echo $row['no_rek']?></td>
    <td class="tg-0pky">Rp. <?php echo number_format(($row['jumlah']),0,'.','.')?></td>
  </tr>
    <?php } ?>
    <tr>
      ...
      <td>Rp. 
      <?php
        $sqlsum="SELECT SUM(jumlah) AS jumlah_masuk FROM tbjurnal;";
        $querysum=mysqli_query($conn, $sqlsum);
        $rowsum=mysqli_fetch_array($querysum);
        echo number_format(($rowsum['jumlah_masuk']),0,'.','.');
      ?>
      </td>
  </tr>
</table>
